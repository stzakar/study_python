...
f=c>5
print (type (f)) #bool
# при равенстве приоритетов операции выполняются слева на права НО при возведении в степень расчитывается справа на лево
# у операции "=" (присвоения) самый низкий приоритет
# операции сравнения "== < > и т.п" bool выполняются после матиматических
#flag=x+7>x+2*x**3**x
#flag=(x+7)>(x+(2*(x**(3**x)))) тажа операция с наглядной последовательностью действий